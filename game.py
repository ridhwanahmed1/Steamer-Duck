...
#  the main game loop etc.
  ...
  """
  Construct serialize object that reads the correct put, at the correct baudrate
  """
ser = serial.Serial(port='/dev/cu.usbmodemSDA447D6E611',baudrate=115200, timeout=1)

""" getSerialData() gets the most recent line of input from the serial communication. 
    It gets rid of all the new line characters
    and returns a clean string that is easy to read """
def getSerialData():
  if ser.isOpen () == False:
    ser.open ()
  b = ser.readline()
  string_n = b.decode('utf-8', 'ignore')
...
class Game:
  ...
  def run(self):
    ...
    """input_thread():
          a function that continously (while the game is running) gets the most recent line from the serial communication
          and using the characters of the string, it will simulate the key presses/holds/releases 
          using the pyninput library.
    """
    def input_thread():
      keyboard = Controller()
      prevkey = Key.enter
    ...
    # Constructs the thread and starts it right before the game officially begins to change states    
    ...
    # join the communication thread because the program has ended
    x.join()
